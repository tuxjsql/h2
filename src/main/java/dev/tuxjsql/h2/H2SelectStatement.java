...
public class H2SelectStatement extends BasicSelectStatement {


    public H2SelectStatement(TuxJSQL tuxJSQL) {
        super(tuxJSQL);
    }
    ...
    private DBSelect select() {
        ...
        if (joinStatement.getJoinType() != null) {
            select += " " + String.format(Queries.JOIN.getString(), H2JoinTypes.getType(joinStatement.getJoinType()).getKey(), joinStatement.getTableTwo().getTable().getName(), table.getName(), joinStatement.getTableOneColumn(), joinStatement.getTableTwo().getName());
        }
